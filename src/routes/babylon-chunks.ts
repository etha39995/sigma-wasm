...
function validateBabylonChunksModule(exports: unknown): WasmModuleBabylonChunks | null {
  ...
  const getTileAtValue = getProperty(exports, 'get_tile_at');
  const clearLayoutValue = getProperty(exports, 'clear_layout');
  const setPreConstraintValue = getProperty(exports, 'set_pre_constraint');
  const clearPreConstraintsValue = getProperty(exports, 'clear_pre_constraints');
  const getStatsValue = getProperty(exports, 'get_stats');
  const generateVoronoiRegionsValue = getProperty(exports, 'generate_voronoi_regions');
  const validateRoadConnectivityValue = getProperty(exports, 'validate_road_connectivity');
